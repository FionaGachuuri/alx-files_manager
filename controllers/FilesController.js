...
class FilesController {
  static async postUpload (request, response) {
    ...
    if (!fileType || !['folder', 'file', 'image'].includes(fileType)) return response.status(400).send({ error: 'Oopsy Missing type!!' });

    const fileData = request.body.data;
    if (!fileData && fileType !== 'folder') return response.status(400).send({ error: 'Oopsy Missing data!!' });
    ...
    if (fileType === 'folder') {
      await dbClient.db.collection('files').insertOne(fileInsertData);
